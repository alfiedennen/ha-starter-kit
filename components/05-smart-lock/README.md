...
On this hardware class, three motor paths behave differently: `lock.lock` is a **silent no-op**, the vendor app's own lock button reports "jammed", but the lock's internal auto-relock timer fires ~35 s after **any** unlock event and locks reliably. So the only remote-lock path is: call `lock.unlock`, and let the lock relock itself. The package implements this behind `input_boolean.lock_use_rearm_trick`. Be honest with yourself about it: **the trick is flaky — it runs my own door every night, with the 90-second fallback nudge there for a reason** — treat `lock.lock` as untrustworthy on a Conexis L2, use the re-arm pattern instead, and treat the fallback phone nudge as the real safety net. Be prepared to walk to the door. If you haven't bought a lock yet, buy one with real remote locking instead.
